...
class FullIntegrationTest extends TestCase
{
  ...
  protected function setUp(): void
  {
                ...
                public function query(string $statement, array $parameters = []): \Cycle\Database\StatementInterface
                {
                  ...
                  return new class($stmt, $statement) implements \Cycle\Database\StatementInterface, \IteratorAggregate {
                    ...
                    public function fetchAll(int $mode = \Cycle\Database\StatementInterface::FETCH_ASSOC): array
                    {
                      ...
                      $rows = array_map(function ($row) use ($schemaCols) {
                        $pdo = $GLOBALS['cycle_orm_test_pdo'] ?? null;
                        if (!$pdo) throw new \RuntimeException('PDO global não definido no fetchAll!');
                        ...
                        if (isset($normalized['id'])) $normalized['id'] = $normalized['id'] !== null ? (int)$normalized['id'] : null;
                        if (isset($normalized['active'])) $normalized['active'] = $normalized['active'] !== null ? (bool)$normalized['active'] : null;
                  ...
                  foreach ($this->values as &$row) {
                    ...
                    foreach ($schemaCols as $col) {
                      if (!array_key_exists($col, $row) || $row[$col] === null) {
                        ...
                        elseif ($col === 'createdAt') $row[$col] = (new \DateTime())->format('Y-m-d H:i:s');
                        elseif ($col === 'description') $row[$col] = null;
                        else $row[$col] = '';
                      }
                    }
                  }
            ...
            public function select(mixed $columns = '*'): \Cycle\Database\Query\SelectQuery
            {
              ...
              $select = new class($driver) extends \Cycle\Database\Query\SelectQuery {
                ...
                public function __construct($driver)
                {
                  ...
                  if (!$this->pdo) throw new \RuntimeException('PDO global não definido!');
                }
    ...
    $this->app->singleton('cycle.schema', fn() => new class {
      ...
      {
        if ($what === \Cycle\ORM\SchemaInterface::ENTITY) return 'CAFernandes\\ExpressPHP\\CycleORM\\Tests\\Fixtures\\TestEntity';
        if ($what === \Cycle\ORM\SchemaInterface::TABLE) return 'test_entities';
        if ($what === \Cycle\ORM\SchemaInterface::DATABASE) return 'default';
        return null;
      }
    });
    // Registrar o serviço 'cycle.migrator' no mock
    $this->app->singleton('cycle.migrator', fn() => new class {});
    // Registrar o serviço 'cycle.repository' no mock
    $this->app->singleton('cycle.repository', fn() => new class {});
  }
